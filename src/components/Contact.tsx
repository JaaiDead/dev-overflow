...
export default function Contact() {
  ...
  const input =
    "w-full clay-input px-4 py-3 font-mono text-sm text-text-primary dark:text-dark-text-primary placeholder-text-dim dark:placeholder-dark-text-dim focus:outline-none transition-all";

  const reveal = (d: number) =>
    `transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"} [transition-delay:${d}ms]`;
    ...
    <section id="contact" ref={ref as React.RefObject<HTMLElement>} className="py-32 relative">
      ...
      <div className="max-w-5xl mx-auto px-6">
        <div className={reveal(0) + " mb-16"}>
          ...
          <p className="font-mono text-sm text-text-dim dark:text-dark-text-dim mt-3">
            Got a project, collab, or just want to chat?
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div className={reveal(100)}>
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                <label className="font-mono text-xs text-text-dim dark:text-dark-text-dim uppercase tracking-widest block mb-2">
                  name
                </label>
                <input
                  type="text"
          ...
          <div className={reveal(200)}>
            <p className="font-mono text-xs text-text-dim dark:text-dark-text-dim uppercase tracking-widest mb-6">
              // or find me at
            </p>
            <div className="space-y-3">
              {CONNECT.map((c) => (
                ...
                >
                  <div>
                    <p className="font-mono text-xs text-text-dim dark:text-dark-text-dim uppercase tracking-widest mb-0.5">
                      {c.label}
                    </p>
                    <p className="font-mono text-sm text-text-primary dark:text-dark-text-primary group-hover:text-accent transition-colors">
                      {c.value}
                    </p>
                  </div>
                  <span className="text-text-dim dark:text-dark-text-dim group-hover:text-accent transition-colors text-sm">
                    ↗
                  </span>
                </a>
              ))}
