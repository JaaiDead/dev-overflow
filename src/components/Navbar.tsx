...
export default function Navbar() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-bg/95 dark:bg-dark-bg/95 backdrop-blur-xl shadow-sm" : "bg-bg/60 dark:bg-dark-bg/60 backdrop-blur-sm"}`}
    >
      <nav className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
        {/* Logo */}
        <button onClick={() => go("home")} className="flex items-center gap-2 group">
          <div className="glass-sm w-8 h-8 flex items-center justify-center font-mono text-accent text-sm font-bold group-hover:shadow-glass-accent transition-all">
            J
          </div>
          <span className="font-mono text-xs text-text-dim dark:text-dark-text-dim group-hover:text-accent transition-colors">
            jaai.dev
          </span>
        </button>
        ...
        <button
          className="md:hidden text-text-dim dark:text-dark-text-dim hover:text-text-primary dark:hover:text-dark-text-primary"
          onClick={() => setOpen((o) => !o)}
        >
